...
\begin{remark}
    Consideriamo il caso non comune in cui un grafo semplice non orientato $G$ non ha
    un nodo di grado almeno $2$.
    
    Allora $A^k = A$ per ogni $k$ dispari: per il Lemma \ref{lemma:numero_cammini},
    $A^k$ conta alla posizione $(i, j)$ il numero di cammini da $i$ a $j$ di lunghezza $k$; ma poiché
    i nodi di $G$ hanno tutti al più grado $1$, i cammini di lunghezza dispari sono solo quelli che vanno da un
    nodo nell'unico nodo adiacente, da cui $A^k = A$. Pertanto vale che
    \begin{equation}
        \label{eq:bound_raggio_spettrale}
        \rho(A) = \lim_{k \to \infty} \lVert A^k \rVert_2^{\nicefrac{1}{k}} \leq
        \lim_{k \to \infty} \lVert A \rVert_2^{\nicefrac{1}{k}} = 1
    \end{equation}

    Ci sono allora soltanto due possibilità per $\rho(A)$:
    \begin{enumerate}
        \item Se $G$ non ha nemmeno nodi di grado $1$, $G$ è il grafo vuoto ed è tale per cui $\rho(A) = 0$.
        \item Se $G$ ha un almeno un nodo di grado $1$, la matrice di adiacenza $A$ ha almeno una colonna $\avec_j$ tale
            per cui $\lVert \avec_j \rVert_2 \geq 1$, da cui $\lVert A \rVert_2 = \rho(A) \geq 1$, dove si è usato che per le
            matrice hermitiane la norma $2$ e il raggio spettrale coincidono. Combinando quest'ultima disuguaglianza  
            con quella dell'eq. \eqref{eq:bound_raggio_spettrale}, si ottiene che $\rho(A)$ è esattamente $1$.
    \end{enumerate}
\end{remark}
...
processore Intel i7-10700K con un clock rate di 3.80 GHz. Tutti i codici sono inclusi anche in \cite{hearotCode}. \medskip

Oltre che su tre casi artificiali, la sperimentazione è stata condotta anche su due casi reali,
utilizzando i grafi \texttt{sp\_500\_2022\_2023} e \texttt{sp\_500\_2019\_2020} \cite{hearotCode},
che rappresentano rispettivamente le matrici di correlazione dei rendimenti degli stock dell'S\&P 500 nel periodo
compreso tra il 2022 e il 2023 e nel periodo compreso tra il 2019 e il 2020, con la prima limitata a un threshold
di $0.2$ e la seconda a un threshold di $0.05$. Le principali proprietà di
questi grafi sono illustrate nella Tabella \ref{tab:prop_sp} e nella Figure \ref{fig:spy_sp} e \ref{fig:spy_sp_2019}.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|c|}       
        \hline
                                     & $\rk(\cdot)$  & $n^2 - \nnz(\cdot)$  & $\rho(\cdot)$ \\
        \hline
        \texttt{sp\_500\_2022\_2023} & $259$ & $16564$ & $\approx 458.07$                  \\
        \hline
        \texttt{sp\_500\_2019\_2020} & $323$ & $23762$ & $\approx 435.63$                  \\
        \hline
    \end{tabular}

    \caption{Proprietà essenziali delle matrici di adiacenza di \texttt{sp\_500\_2022\_2023} (taglia $n = 487$) e \texttt{sp\_500\_2019\_2020} (taglia $n = 487$): rango ($\rk(\cdot)$), numero di elementi nulli ($n^2 - \nnz(\cdot)$) e raggio spettrale ($\rho(\cdot)$).}
\end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{images/sp_spy.png}
    \caption{Sparsità del complementare della matrice di adiacenza del grafo \texttt{sp\_500\_2022\_2023} di densità $93\%$. In blu sono indicati gli elementi non nulli
    del complementare (e quindi quelli nulli della matrice di adiacenza). Per semplicità e chiarezza è visibile solo il minore principale dettato dalle prime $100$ colonne e dalle
    prime $100$ righe.}
    \label{fig:spy_sp}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{images/sp2019_spy.png}
    \caption{Sparsità del complementare della matrice di adiacenza del grafo \texttt{sp\_500\_2019\_2020} di densità $90\%$. In blu sono indicati gli elementi non nulli
    del complementare (e quindi quelli nulli della matrice di adiacenza). Per semplicità e chiarezza è visibile solo il minore principale dettato dalle prime $100$ colonne e dalle
    prime $100$ righe.}
    \label{fig:spy_sp_2019}
\end{figure}

\subsection{Generazione dei grafi artificiali}
...
\begin{table}[H]
    ...
    \caption{Proprietà essenziali delle matrici $A_{94}$, $A_{98}$ e $A_{99.5}$ di taglia $n = 15000$: rango ($\rk(A)$), numero di elementi nulli ($n^2 - \nnz(A)$) e raggio spettrale ($\rho(A)$).}
\end{table}
...
$A_{99.5}$; oltre che sulle matrici di adiacenza di \texttt{sp\_500\_2022\_2023} e \texttt{sp\_500\_2019\_2020}.
Si è scelta una tolleranza \texttt{tol} pari
a $10^{-8}$ e un numero massimo di iterazioni \texttt{maxit} pari a
...
\begin{table}[H]
    ...
    \resizebox{\textwidth}{!}{
        \begin{tabular}{|l|cc|cc|cc|}
            \hline
            \multirow{2}*{$A$/grafo} & \multicolumn{2}{|c|}{Codice \ref{code:katz_classic}} & \multicolumn{2}{|c|}{Codice \ref{code:katz_complement}} & \multicolumn{2}{|c|}{Codice \ref{code:katz_complement_no_loops}} \\
            ...
            \hline
            \texttt{sp\_500\_2022\_2023} & $7$ & $\approx 1.34 \cdot \mathbf{10^{-3}}$ & $6$ & $\approx 3.11 \cdot \mathbf{10^{-4}}$ & $6$ & $\approx 2.33 \cdot \mathbf{10^{-4}}$ \\
            \hline
            \texttt{sp\_500\_2019\_2020} & $7$ & $\approx 1.03 \cdot \mathbf{10^{-3}}$ & $6$ & $\approx 3.18 \cdot \mathbf{10^{-4}}$ & $6$ & $\approx 2.99 \cdot \mathbf{10^{-4}}$ \\
            \hline
        \end{tabular}
    }

    \caption{Numero di iterazioni e tempi di esecuzione per calcolare iterativamente il ranking di Katz su $A_{94}$, $A_{98}$, $A_{99.5}$, \texttt{sp\_500\_2022\_2023} e \texttt{sp\_500\_2019\_2020} con le funzioni dei Codici \ref{code:katz_classic},
