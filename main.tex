...
Oltre che su tre casi artificiali, la sperimentazione è stata condotta anche su un caso reale,
utilizzando il grafo \texttt{sp\_500\_2022\_2023} \cite{hearotCode},
che rappresenta la matrice di correlazione dei rendimenti degli stock dell'S\&P 500 nel periodo
compreso tra il 2022 e il 2023, limitata a un threshold di $0.05$. Le principali proprietà di
questo grafo sono illustrate nella Tabella \ref{tab:prop_sp} e nella Figura \ref{fig:spy_sp}.
...
\begin{table}[H]
    ...
    \caption{Proprietà essenziali delle matrice di adiacenza di \texttt{sp\_500\_2022\_2023} di taglia $n = 487$: rango ($\rk(\cdot)$), numero di elementi nulli ($n^2 - \nnz(\cdot)$) e raggio spettrale ($\rho(\cdot)$).}
\end{table}
...
$A_{99.5}$; oltre che sulla matrice di adiacenza di \texttt{sp\_500\_2022\_2023}. Si è scelta una tolleranza \texttt{tol} pari
...
\begin{table}[H]
    ...
    \caption{Numero di iterazioni e tempi di esecuzione per calcolare iterativamente il ranking di Katz su $A_{94}$, $A_{98}$, $A_{99.5}$ e \texttt{sp\_500\_2022\_2023} con le funzioni dei Codici \ref{code:katz_classic},
